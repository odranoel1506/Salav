...
class MySqlBackupLite
{
    ...
    private function insertStaments($numjk)
    {

        foreach ($this->arrayTables as $table) {
            $this->sqlInsertStaments($table, $numjk);
        }
    }



    private function sqlInsertStaments($table, $numjk)
    {

        $this->getTableData($table, $numjk);
    ...
    private function getTableData($table, $numjk)
    {
        $ofset = $numjk;
        $this->queryResult    = $this->mySqli->query('SELECT * FROM `' . $table . '` limit ' . $ofset . ',100000');
...
try {
    ...
    $cantij    = $mySqli->query('SELECT count(*) FROM `consulta`');
    ...
    for ($i = 0; $i < $cantidaddiv; $i++) {
        ...
        $bck->backUp($canidaincrementable);
        $bck->setFileDir('');
    ...
    $tmp_file = 'manchasnuevo_'.$nombrearchivo.'.zip';
    $files = glob('manchasnuevo_'.$nombrearchivo.'.zip'); //obtenemos todos los nombres de los ficheros
    ...
    if ($zip->open($tmp_file,  ZipArchive::CREATE)) {
        for ($i = 0; $i < $cantidaddiv; $i++) {
            ...
            $zip->addFile('backupFile' . $i + 1 . '.txt');
        }
