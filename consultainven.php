...
class MySqlBackupLite
{
    ...
    public function backUp($numjk,$suc)
    {
    ...
    private function insertStaments($numjk,$suc)
    {

        foreach ($this->arrayTables as $table) {
            $this->sqlInsertStaments($table,$numjk,$suc);
        }
    }



    private function sqlInsertStaments($table,$numjk,$suc)
    {

        $this->getTableData($table,$numjk,$suc);
    ...
    private function getTableData($table,$numjk,$suc)
    {
        $ofset = $numjk;
...
$canidaincrementable=0;
    ...
    for ($i = 0; $i < $cantidaddiv ; $i++) {
        ...
        $bck->backUp($canidaincrementable,$suc);
        $bck->setFileDir('');
    ...
    $date=date('D_F_ Y_h_i_s_A');

    $tmp_file = 'manchasinventario_'.$nombrearchivo.'.zip';
    $files = glob('manchasinventario_'.$nombrearchivo.'.zip'); //obtenemos todos los nombres de los ficheros
    ...
    if ($zip->open($tmp_file,  ZipArchive::CREATE)) {
        for ($i = 0; $i < $cantidaddiv ; $i++) {
            ...
            $zip->addFile('backupFileinventario' . $i+1 . '.txt');
